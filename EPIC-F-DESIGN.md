...
| AI worker picks up an `AiGeneration` row whose `createdAt > 24h` ago (A5 / POST-EPIC-F §5) | Worker `processAiGenerateJob` STEP 1.5 gates on `Date.now() - row.createdAt > 24h` (strict `>`).  Stale → single atomic `markFailed(lastError='generation_stale')` UPDATE, no `$transaction` wrapper (no related audit, unlike `handleProviderError`'s two-write pattern), worker returns normally — BullMQ marks the job complete and does NOT retry.  No LLM call, no `AiSpendBucket` increment, no `Message.generatedText` update.  `'generation_stale'` is a `lastError` string (same pattern as `'content_filter'`), NOT a registered `AUDIT_ACTIONS` constant.  Rationale: Q9's retry tail through a multi-hour provider outage can drift into the next day; a winback sent 24+ hours after the actual state transition anchors to an event the customer experienced yesterday — semantically broken.  **§5's "non-retryable in the BullMQ error classifier" instruction is superseded**: the worker returns (no throw), so `isRetryable` (which reads `error.retryable` — no "list") is never invoked.  No typed error class. |
