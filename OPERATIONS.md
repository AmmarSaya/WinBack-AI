...
- [ ] **GDPR compliance webhooks declared via `shopify.app.toml`.** The `[webhooks.privacy_compliance]` block in `shopify.app.toml` declares all 3 URLs (`customer_data_request_url`, `customer_deletion_url`, `shop_deletion_url`) pointing at `https://winback-ai-web.onrender.com/webhooks`. Pushed to Partners via `shopify app deploy` — current Partners-side app version is the most recent `winback-ai-<N>` from that deploy. **Verify in Partners Dashboard** → Winback AI → App Settings → Compliance Webhooks that all 3 URLs are listed and active. (Install code path EXCLUDES GDPR topics from `webhookSubscriptionCreate` per I-1 fix; the app-level `privacy_compliance` declaration is what makes Shopify deliver these topics. M-7 closed 2026-05-23 — toml is now canonical source-of-truth for app-level config.)
