...
- **Partial resolution 2026-06-05 (B5, `bf3b61c`):** `/readyz` now probes Postgres `SELECT 1` + Redis `PING` + outbox DLQ depth + stall age. 503 for the first two (routing signal); 200-with-`warnings[]` body for DLQ/stall (operator-alert signals, never routing). Render Health Check Path → `/readyz` wired post-merge.
- **Remaining gap — DLQ/stall body-content alerting deferred:** Render notifications are event-based only, not body-content. A status-only alert at `/readyz` catches Postgres / Redis outages (503) but NOT DLQ backlog or stall escalations (200 + warnings). Empirically confirmed in Render dashboard 2026-06-05 — Notifications has three coarse options (All / Only failure / None), no per-event-type granularity, no body-content matching. Acceptable at design-partner scale (manual `/readyz` curl check cadence; recoverable via `pnpm cli:outbox:replay`). **Revisit at Epic G batch 8.1 design** — when `dispatch.send` ships, DLQ depth becomes revenue-impacting (failed sends = lost recovered revenue) and one of three mechanisms must be chosen: external monitor (UptimeRobot / Better Uptime with body-content matching) / follow-up `/operatorz` route that 503s on DLQ (semantically a misuse of 503 but works around the gap) / Render's improved alerting if it ships in the meantime.
