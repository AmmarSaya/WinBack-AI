...
- **Paired operator step:** Partners → Winback AI → App Settings → Compliance Webhooks → register the 3 URLs at `https://winback-ai-web.onrender.com/webhooks` (see OPERATIONS.md §4 checklist item). Required before App Store submission AND before first real merchant install. Without the dashboard step Shopify never delivers the GDPR topics; without the probe-side fix the dashboard refuses the save.
