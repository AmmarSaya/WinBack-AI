...
describe('getQueues + closeQueues', () => {
  it('1. returns an object with outboxDrain and attributionCompute Queue instances using QUEUE_NAMES values', () => {
    const queues = getQueues();

    expect(queues.outboxDrain).toBeDefined();
    expect(queues.attributionCompute).toBeDefined();

    // BullMQ's Queue constructor was called twice with the canonical
  ...
  it('2. is memoized — repeated calls return the same Queues reference, single Queue + Redis client construction', () => {
    ...
    // Queue constructor called exactly twice total (once per queue, on
  ...
  it('3. closeQueues closes both Queue instances + the shared client, clears the cache, and re-initializes fresh on next getQueues()', async () => {
    const first = getQueues();
